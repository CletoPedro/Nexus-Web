...
class MemoryService:
    ...
    async def create_memory(
        self, *, content: str, tags: list[str] | None = None, metadata: dict | None = None
    ) -> Memory:
        if not content or not content.strip():
            raise ValidationError("Memory content cannot be empty.")
        memory = Memory(content=content.strip(), tags=tags or [], metadata_=metadata or {})
    ...
    async def update_memory(
        ...
        *,
        ...
        if not content or not content.strip():
            raise ValidationError("Memory content cannot be empty.")
        existing = await self.get_memory(memory_id)  # raises NotFoundError
    ...
    async def delete_memory(self, memory_id: uuid.UUID) -> None:
        await self.get_memory(memory_id)  # raises NotFoundError if missing
        await self._repo.delete(memory_id)

    async def search_memories(self, query: str, *, limit: int = 50) -> list[Memory]:
